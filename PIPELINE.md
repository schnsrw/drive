...
| # | Item | Brief | Priority | Trigger |
|---|---|---|---|---|
| RT1 | `PresenceHub` (in-process), SSE endpoint, heartbeat, leave. **Phases 1a + 1b shipped** — `crates/drive-http/src/presence.rs` holds the in-process state (per-workspace `WorkspaceChannel { entries, broadcast::Sender }`), 60 s TTL sweep, membership-gated `POST /api/presence/{ws}/beat` + `…/leave`, **plus the `GET /api/presence/{ws}` SSE stream**: subscribe-before-snapshot pattern, initial `Present` burst, then forwards every beat / leave / sweep-expiry event over a tokio broadcast bus. 10/10 unit tests covering hub mechanics + cross-workspace isolation. **Remaining**: 1c — audit-emit middleware extension publishes `action` events to the workspace channel; 1d (deferred polish) per-user concurrent-stream cap of 5 + heartbeat rate limit (1/10 s upper bound). | [`14-presence`](./docs/research/14-presence.md) | P1 | 1b unblocks RT2 / RT3 SPA wiring; 1c rides on the existing audit-emit path |
