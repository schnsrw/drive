...
| ED4 | **Excalidraw editor for `.excalidraw` files** — third co-edit format alongside `.docx` / `.xlsx`. Drop `@excalidraw/excalidraw` into the SPA, render inside the existing Preview modal (same shell as `CasualDocEditor` / `CasualSheetWorkspace`). Storage round-trips JSON via the existing `GET / PUT /api/files/{id}/content` endpoints — no WOPI, no token mint (Excalidraw doesn't need lock semantics for v0; collab can wake later via the same Yjs path NT9 reserves). New MIME `application/vnd.excalidraw+json`; `inferKind` gains an `excalidraw` bucket with its own thumbnail. "New diagram" entry slots into the Sidebar's New menu next to Document / Spreadsheet. Brief: `docs/research/19-excalidraw.md` (TODO). | — (needs brief) | P2 | User asked 2026-06-10; needs brief + Excalidraw package audit (size, license — MIT, peer deps on react-dom 19) before scheduling |
