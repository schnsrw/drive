...
| SR14 | A11y polish — combobox role + `aria-controls` + `aria-activedescendant` on the search input shipped; count chip + infinite-scroll already on `aria-live="polite"`; filter chips already `aria-pressed`. Remaining: focus-trap inside SearchToolbar's filter-chip popovers (Radix Popover or a manual trap), `prefers-reduced-motion` audit on the popover keyframes. | [`12-search-surface`](./docs/ux/12-search-surface.md) §"Accessibility" | P1 | Run alongside each SR feature; popover focus-trap is the last chunk |
...
| NT5 | Block handle — Phases 1 + 2a shipped (hover-revealed handle, click menu Duplicate / Move up / Move down / Delete, **drag-to-reorder** via the handle — ProseMirror Dropcursor draws the insertion indicator, slice is `move:true` so the source is removed on drop; desktop-only). Remaining: "Turn into →" sub-menu (Heading / List / Quote / Code). | [`17-notes-general-user-ux`](./docs/research/17-notes-general-user-ux.md) §"Drag handle" | P2 | "Turn into" is the last piece |
