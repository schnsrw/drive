...
- **Click model: single = preview modal, double = editor** (user directive, 2026-06-16). The earlier "editor-eligible types route on single click" approach was wrong: it collapsed the menu trio (Open / Preview / See details — they all jumped to the editor) and the user couldn't *peek* at a `.docx` or `.xlsx` without launching the heavy editor chunk. New model is uniform across every file kind: single left-click on any card → `PreviewModal` (metadata + preview); double left-click on any card → `/file/<id>` editor view (FileFullscreen already branches on `inferKind` to mount the right SDK / viewer / downloader). Menu wiring follows: `Open` → editor route, `Preview` → modal, `See details` → modal today (Phase 2 will surface a dedicated panel with shared-with + role management + audit log per file). Implementation note: the browser fires `click` before `dblclick`, so the single-click action is held behind a 250 ms timer keyed on a single `singleClickTimerRef`; `dblclick` cancels that timer before pushing the route. Tests flipped from `.click()` → `.dblclick()` for the editor-route assertions.

- **Slate Console consistency sweep — last drift caught** (BUG-CONSISTENCY). Five concrete leftovers from the pre-reskin warm-paper / gold palette were still rendering against the new cool-slate / cyan tokens, breaking visual continuity. `PreviewModal.tsx` nav-arrow circles flipped `rgba(251,250,246,.9)` → `var(--card)` + `var(--line-strong)`. `Files.tsx` folder hover-hint chip flipped the same warm-paper → `var(--card)`. `Logo.tsx` SVG fallback cream `#F2F0EA` → `#F6F8FA` to match the Slate paper. `FileThumb.tsx` sheet thumbnail cells flipped the green `rgba(46,140,90,…)` borders + tinted-cell background to cyan `rgba(8,145,178,…)`. `AboutSection.tsx` copy updated from "Fraunces and Hanken Grotesk" to "IBM Plex Sans and IBM Plex Mono" to match the actual fonts shipping in `tokens.css`. Closes BUG-CONSISTENCY in PIPELINE.

- **File-card kebab + right-click context menu re-wired** (BUG-KEBAB-INVISIBLE + BUG-RIGHT-CLICK). Two regressions from the Slate Console reskin landed together because they share the same surface: (a) `kebabStyle()` still hardcoded the pre-reskin warm-paper `rgba(251,250,246,.92)` background, which blends invisibly against the new cool-paper `#F6F8FA` card. Swapped to `var(--card)` + `var(--line-strong)` border + a faint slate shadow so the kebab reads as a discoverable surface in both themes. Hover transitions to `var(--bg-hover)` with `--accent` border instead of the old warm fallback. (b) `<Card>` and `<ListRow>` were plain function components — Radix's `<ContextMenu.Trigger asChild>` couldn't inject `onContextMenu` / `ref` onto a non-forwardRef child, so right-click silently no-op'd. Both rewritten as `React.forwardRef`'s that spread `...rest` through to the underlying `<div>`. Closes BUG-KEBAB-INVISIBLE + BUG-RIGHT-CLICK in PIPELINE.

- **Editor-eligible files (`.docx` / `.xlsx`) skip the preview modal entirely** (BUG-EDITOR-DIALOG-DEFAULT). The user pushed back multiple times on the modal-first flow — clicking a file you intend to *edit* should take you to the editor, not to a preview-with-"Open in editor"-button. `Files.tsx` now branches at every open site (grid card click, list row click, `cd:open-file` Cmd-K event, kebab → Open / Preview / Details) on `inferKind`: `doc` / `sheet` route directly to `/file/<id>` (editor mode); everything else (PDF, image, video, raw text, generic) still gets the PreviewModal. Cold-load of unknown files via the open-by-id event also branches on the fetched metadata — editor types push history without ever rendering the modal. Updated `_iframe-verify.spec.ts` + `sdk-integration.spec.ts` to assert the new direct-route behavior; the "Open in editor" intermediate-click is gone.

### Fixed

- **MK-PERF-95 — Lighthouse perf gate restored to ≥0.95.** Local re-baseline against CI's exact throttle profile (`cpuSlowdownMultiplier: 4`, mobile, 3 runs) returned **perf=1.00 on both index + docs/install** — the PIPELINE row's "Plex Variable is ~95KB vs Inter's ~62KB" was wrong. Actual file sizes: Plex Variable latin = 45.7 KB, Inter Variable latin = 48.3 KB; Plex is *smaller*. Bumped `marketing/lighthouserc.json` from `0.85` → `0.95`; 5pt headroom over local. Also bumped dark-mode `--muted-2` from `#5e6e82` → `#7a8ca1` to clear WCAG AA (was 3.59:1 against `#0b1220`; now ≥4.5:1) — clears the contrast warning that was firing on every Compare / Footer secondary text in dark mode. The remaining contrast warning is Astro's default Shiki `github-dark` theme rendering comments at `#6A737D` on `#24292E` (3.04:1) inside docs `<pre>` blocks — separate cleanup, tracked in PIPELINE.
- **CI iframe-verify — green again (5-deep red).** Two pre-existing failures were stacking under unrelated commits. (1) The bundled SDK embed runtimes were importing `from "crypto"` as a bare specifier — the browser can't resolve those without an importmap, every preview-mount fired a `pageerror` that the strict iframe-verify spec rethrows. `scripts/copy-embed.mjs` now post-processes the copied `embed.html` and, when the runtime references the bare specifier, injects a single `<script type="importmap">` mapping `"crypto"` to a small data-URL ES module that re-exports WebCrypto's `webcrypto` / `randomUUID` / `getRandomValues` plus a synchronous `createHash('sha256')` polyfill (FIPS 180-4 reference impl; verified against `SHA256("hello") = 2cf24dba…`). A sibling inline `<script>` defines `globalThis.process` / `globalThis.global` / a minimal `Buffer` (Uint8Array-backed with `from` / `alloc` / `concat` / `isBuffer` / `toString` for hex / base64 / utf8). All three shims are conditional on the SDK runtime actually using the surface — `@schnsrw/casual-sheets@0.5.6` dropped the crypto import upstream, so its `embed.html` now ships unmodified; `@schnsrw/docx-js-editor@1.1.1` still needs it. (2) The `sdk-integration.spec.ts` referenced `getByTestId('sheet-workspace')` — drifted three months ago when SDK Phase 1.5 renamed the wrapper to `casual-sheet-workspace`. Renamed. Full Playwright run: 25 passed, 1 skipped.
...
- **Design re-skin Phase D — marketing components on Slate Console.** Marketing was already token-driven; Phase A's `marketing/src/styles/tokens.css` swap auto-flipped Hero gradients, FeatureGrid, Compare, ScreenshotShowcase, FinalCta, Nav, Footer, and the docs pages. Only one warm-paper hex remained — the SVG `<g fill="…#F2F0EA">` fallback inside `Logo.astro` — replaced with the new `#F6F8FA` paper. Builds clean (`pnpm build` 7 pages, 0 warnings). The `MK-PERF-95` row picks up a new sub-bullet: **Plex Variable is heavier than Inter** (~95 KB latin range vs ~62 KB), so the cold-paint budget tightened — restoring 0.95 likely needs Plex subsetting or per-page weight slicing. Picked up before the next marketing-facing release.
- **Design re-skin Phase C — workspace component touch-ups.** Sweep over every remaining hardcoded warm-paper value the SPA still carried from the gold era. All `rgba(26,26,30,…)` (old ink) → `rgba(15, 23, 42, …)`. All `rgba(176, 69, 69, …)` (old danger) → `rgba(220, 38, 38, …)`. All `rgba(67, 107, 177, …)` (old info-blue) → `rgba(37, 99, 235, …)`. All `rgba(242,240,234,…)` / `rgba(243,241,234,…)` (old paper) → `rgba(232, 237, 242, …)`. Avatar monograms in Settings → Profile and Settings → Members flip from the dark-grey gradient to the same cyan gradient the sidebar uses — same primary, same brand chip everywhere it shows. FileThumb's procedural image / video gradients flip from the warm rainbow to a cyan-leaning set (cyan → blue → navy) per the design system §"Imagery / thumbnails". The Activity pill palette stops aliasing custom hex codes — gold → `--accent-hover`, blue → `--info`. Warm-linen `#E7E4DC` placeholders → `var(--bg-subtle)`. SelectionBar shadow tightens to slate-tinted rgba.
- **Design re-skin Phase B — Sidebar dark rail.** The left sidebar now stays deep slate (`--rail #0F1A2A`) regardless of light/dark theme — the "Slate Console" framing the design system locked in. Every consumer of the rail flipped over to the matching tokens: section labels → `--rail-muted`, idle nav rows → `--rail-text` on transparent, **active nav row → cyan wash (`--rail-active`) + bright-cyan ink (`--rail-active-text`)**, hover → thin white-wash, the New button picks up the cyan fill + cyan-tinted `--shadow-button` lift on hover, the storage card lifts via `--rail-2` with a cyan-gradient progress fill, and the AvatarRow gets a cyan-gradient monogram chip. `Wordmark` gains a `tone="rail"` prop that inherits `currentColor` for "Casual" + uses `--rail-muted` for "DRIVE". `Logo` already pivoted on `currentColor` for the square — wrapping it in `color: var(--accent)` flips the chrome to cyan-on-paper-cloud. WorkspaceSwitcher trigger + ThemeToggle re-skin to match the rail. (Phase C — workspace surface components — and Phase D — marketing — still pending.)
- **Design re-skin Phase A — Slate Console tokens + IBM Plex Sans.** First wave of the visual pivot locked in the `project_design_reskin_slate_console` memory entry from the 2026-06-12 `claude.ai/design` handoff bundle. **Palette flip**: warm paper + gold → cool slate + cyan. `--paper #F6F8FA`, `--ink #0F172A`, `--card #FFFFFF`, `--accent #0891B2` (cyan-600, fills primary buttons + active nav), `--shadow-button` cyan-tinted for the New button. **New `--rail-*` token group** for the (Phase B) dark sidebar treatment — `--rail #0F1A2A`, `--rail-text`, `--rail-active`. **Typography flip**: Fraunces + Hanken Grotesk → IBM Plex Sans (display + body) + IBM Plex Mono (code). SPA pulls via `@fontsource-variable/ibm-plex-sans` + `@fontsource/ibm-plex-mono`; marketing self-hosts via woff2 files under `public/fonts/` fetched at CI time from jsDelivr (replacing the Inter fetch). Dark mode mirrors every key — cyan brightens to `#22C3DE` on a deep-navy base. SR4 density vars, `cd-popover-in` / `cd-skeleton` keyframes, and the `@theme` Tailwind bridge all carry forward. Phase B (Sidebar dark rail) + C (component touch-ups) + D (marketing components on the new tokens + MK-PERF-95 revisit) follow.
- **Workspace invitations — magic-link auto-create (MU1 Phase 1d). MU1 row closes.** Anonymous accepts no longer 401. The accept handler now takes `OptionalAuthSession`: when there's no session, the server mints a fresh `user-<short-ulid>` user (random throwaway Argon2id hash they don't know — they can set a real password later via Settings → Profile or stay OIDC-only), starts a session, sets the `__Host-cd_sid` cookie, and returns the new identity in a `created_user` field on `AcceptResp`. The SPA's `InviteAccept` page lost the "Sign in to join" branch — every visitor gets "Join workspace", and the success toast announces the auto-generated username when one was minted ("Welcome to <Workspace>. Your username is user-abc123."). After accept, the SPA calls `refresh()` on AuthContext so the new session cookie hydrates `/api/me` without a reload. New audit event `auth.magic_link_create`. Workspace.joined audit + RT4 presence broadcast still fire so existing members see the arrival. New integration test `anonymous_accept_mints_user_session_and_membership` verifies the end-to-end shape (cookie set, user mint, /api/me reflects the new user). Fixed in same commit: `PresenceContext.beatNow` was firing the `/beat` POST in DEMO_MODE, leaving Vite proxy errors in the console that broke the strict `_iframe-verify` e2e check; gated on `!DEMO_MODE`. **Pre-existing CI red on the `_iframe-verify` / xlsx-preview tests** persists from commit 65cd2ab (`@schnsrw/casual-sheets@0.5.1` ships a `crypto` module import that fails in the browser's iframe); unrelated to MU1.
- **Workspace invitations — Members tab (MU1 Phase 1c).** Settings → Members section is now real (was a ComingSoon panel). Two stacked cards: **Active members** (monogram avatar + role chip — Owner badge on the workspace owner; per-member remove is reserved for MU2's role-tier overhaul) and **Pending invitations** (status badges that compute live: active / exhausted / expired / revoked, with `X / Y used` + relative `Created N ago / Expires in N` timestamps). Active invites carry a Revoke button gated behind a destructive `ConfirmDialog`; revoke fires the existing `DELETE /api/workspaces/{id}/invitations/{inv_id}` and refreshes. Personal workspaces render a friendly placeholder ("This is your personal workspace — it's just for you") instead of an empty members card.
- **Workspace invitations — SPA UI (MU1 Phase 1b).** Three pieces. (1) `InviteDialog` Radix modal: expiry (24h / 7d / 30d / Never) + reuse radio (One person / Multiple people with a `2..1000` numeric cap), POSTs to `/api/workspaces/{id}/invitations`, displays the resulting URL with a copy button that auto-selects on focus. (2) `WorkspaceSwitcher` dropdown footer gains "Invite to <Workspace>" entry — visible only on team workspaces (hidden on personal + in DEMO_MODE). (3) `/invite/<token>` `InviteAccept` route mounted BEFORE the auth gate so the anonymous-safe peek payload (workspace name + inviter + role + remaining-uses) renders without sign-in; signed-in visitors get "Join workspace" → POST accept → switch active workspace + route to `/`; anonymous visitors get "Sign in to join" → bounce with the invite path stashed in `sessionStorage`. New API client fns: `createInvitation`, `listInvitations`, `revokeInvitation`, `peekInvitation`, `acceptInvitation`.
- **Workspace invitations — backend (MU1 Phase 1a).** New `workspace_invitations` table (migration 0013) carries the token + role + expires_at + max_uses + used_count + revoked_at. `WorkspaceInvitationRepo` exposes insert / find_by_token / find_by_id / list_for_workspace / revoke / try_consume (the atomic accept primitive). Five HTTP endpoints land on `drive-http`: `POST /api/workspaces/{id}/invitations` (member-gated create — `member` role only in 1a; `admin` rejected with 400 until MU2 ships role tiers), `GET /api/workspaces/{id}/invitations` (member-gated list), `DELETE /api/workspaces/{id}/invitations/{inv_id}` (member-gated revoke), `GET /api/invitations/{token}` (anonymous-safe peek — workspace name + inviter + role + remaining uses, never the token), `POST /api/invitations/{token}/accept` (signed-in accept; idempotent already-member path; 256-bit token via OsRng; constant-time-equivalent indexed lookup). Accept emits `workspace.joined` audit + a presence broadcast (RT4 toast surfaces the arrival). Magic-link auto-create (anonymous → mint user + session) deferred to 1d. 8 integration tests cover create / list / revoke / non-member 403 / signed-in accept / single-use exhaustion / multi-use admits-N / unknown-token 404 / admin-role 400.
- **Search — no-results recovery panel (SR12 partial).** When a search comes back empty AND at least one filter chip is active, the empty state is replaced by a polished panel listing one-click relaxations: Drop the Type filter / Drop the Owner filter / Widen Modified to All time / Widen Created to All time / Drop the Size filter / Drop the share-link filter / Search in trash too / Search across all workspaces. Each button mutates `searchFilters` and the existing search effect re-fires immediately. Computes relaxations from active filters; renders null when nothing's actionable so a query-only empty search keeps the generic "Try a different search" copy. Did-you-mean half deferred — that needs OpenSearch's `phrase_suggester` (S2-blocked).
- **Presence SPA — quiet action toast (RT4).** Files.tsx now watches `usePresenceActions()` and fires a 3-second sonner toast when a peer renames / trashes / uploads / creates-folder on a target that's currently in the user's grid. Self-actions and out-of-view targets are silently skipped. After the toast lands, the local grid refreshes so the row reflects the new name. `lastSeenActionTsRef` dedupes across renders. Maps `files.rename` / `folders.rename` → "renamed", `files.trash` → "moved to trash", `files.upload` → "uploaded", `folders.create` → "created folder".

### Fixed

- **Theme toggle was never mounted in the SPA — dark/light auto-flipped with no recourse.** `ThemeToggle` existed as a component but no surface mounted it, so the SPA silently followed `prefers-color-scheme` on every OS theme change. Mounted in the Sidebar footer next to AvatarRow. Fixed the component's broken CSS-var references (`--fg-muted` / `--dur-fast` / `--ease-out` / `--radius-md` → tokens that actually exist in `tokens.css`). Title attribute spells out the current state + what the next click does ("Dark theme (click for system)"). Click cycle: light → dark → system → light. Storage key `theme`, written to `localStorage`, applied to `<html data-theme>`; "system" removes the attribute so the `@media (prefers-color-scheme: dark)` block in tokens.css takes over.

- **Presence SPA — file-row viewing dot (RT3).** `FileViewingDot` component (7 px, peer's tint, white outline ring) subscribes to `useViewingFile(fileId)` from PresenceContext; renders `null` when no peer is viewing. Mounted in both `FileCard` (top-left of the thumbnail) and `ListRow` (just left of the filename label). PreviewModal and FileFullscreen now both call `useReportViewing(fileId)` on mount / file-change → `null` on close, so a peer opening a file lights up the dot for everyone else within one beat tick. Tooltip via `title=` carries the viewer's name.
- **Presence SPA — PresenceContext + AvatarStack (RT2).** `web/src/state/PresenceContext.tsx` provider mounts at the Shell root, subscribes to `GET /api/presence/{ws}` via `EventSource` per active workspace, beats every 25 s with the file currently being viewed, posts `/leave` via `navigator.sendBeacon` on `pagehide`, and reduces `present` / `left` / `action` SSE events into a `Map<user_id, PresenceUser>`. Exposes hooks: `usePresenceUsers` (self-excluded, sorted most-recent-first), `useViewingFile(id)` (RT3 will consume), `useReportViewing()` (preview modal will hook in), `usePresenceActions` (RT4 toast). `AvatarStack` renders the first 5 monogram-on-tint avatars + `+N` overflow chip in the Sidebar, directly below the workspace switcher. Quiet by default — collapses to nothing when no one else is present. Demo mode skips the subscription so the static demo doesn't spam reconnect attempts.
- **Presence backend Phase 1c — action broadcast (RT1).** `PresenceEvent` gains an `Action` variant ({ user_id, action, target_id, target_name }). New `PresenceHub::broadcast_action` helper wired into the file/folder mutation handlers — `files.upload`, `files.rename`, `files.trash`, `folders.create`, `folders.rename` all publish to the workspace channel alongside the existing `AuditRepo::emit`. SSE subscribers see `event: action` frames; the SPA's PresenceContext routes by action string (RT2 / RT3 / RT4 unblocked). Broadcast to a workspace with no entries / subscribers is a quiet no-op. Two new unit tests cover the happy path + the inactive-workspace edge case.
