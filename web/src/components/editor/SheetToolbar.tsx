...
export function SheetToolbar({
  ...
  // Helper that the children call so each subcomponent gets a stable
  // dispatch function whose closure already includes the args. The
  // SDK ref's typings expose `executeCommand(command, args?)` after
  // the 0.7 bump.
  function dispatch(cmd: Command, args?: CommandArgs) {
    (
      iframeRef.current as unknown as {
        executeCommand: (c: Command, a?: CommandArgs) => void;
      } | null
